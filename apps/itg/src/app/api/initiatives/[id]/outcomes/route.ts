...
export async function POST(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const ins = await db
      .prepare(
        `INSERT INTO initiative_outcomes
         (initiative_id, title, metric_json, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`,
      )
      .bind(
        initiativeId,
        title.trim(),
        typeof metric_json === "string" ? metric_json : metric_json ? JSON.stringify(metric_json) : null,
        typeof status === "string" && status ? status : "defined",
        now,
        now,
      )
      .run();
    ...
    const created = await db.prepare("SELECT * FROM initiative_outcomes WHERE id = ?").bind(outcomeId).first();
    return NextResponse.json({ outcome: created });
  } catch (error) {
    console.error("[initiatives/:id/outcomes] POST error:", error);
    return NextResponse.json(
      { error: "Failed to create outcome", message: error instanceof Error ? error.message : String(error) },
      { status: 500 },
    );
  }
}
